...
def get_detailed_batter_stats(year: int) -> pl.DataFrame:
    ...
    df = df.with_columns(
        pl.struct(pl.all())
        .map_elements(calculate_woba, return_dtype=pl.Float64)
        .alias("wOBA")
    )
    ...
    df = df.with_columns(
        pl.struct(pl.all())
        .map_elements(lambda row: calculate_wrcplus(row), return_dtype=pl.Float64)
        .alias("wRC+")
    )

    xdf: pl.DataFrame = pl.from_pandas(
        pyb.statcast_batter_expected_stats(year=year, minPA=1)
    )
    ...
    final_df = final_df.rename(
        {"est_woba": "xwOBA", "player_id": "playerID", "BA": "AVG"}
    )
...
if __name__ == "__main__":
    ...
    results_df: pl.DataFrame = get_detailed_batter_stats(default_year)
    print(f"Batter Stats for {default_year}:")
